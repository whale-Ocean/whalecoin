...
The data directory is the default location where the WhaleCoin Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.WhaleCoin/`
macOS    | `$HOME/Library/Application Support/WhaleCoin/`
Windows  | `%APPDATA%\WhaleCoin\` <sup>[\[1\]](#note1)</sup>
...
`./`               | `bitcoin.conf`        | Contains [configuration settings](bitcoin-conf.md) for `whalecoind` or `bitcoin-qt`; can be specified by `-conf` option
`./`               | `whalecoind.pid`        | Stores the process ID (PID) of `whalecoind` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `whalecoind` or `bitcoin-qt`; can be specified by `-debuglogfile` option
